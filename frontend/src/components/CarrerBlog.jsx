...
const MOCK_BLOGS = [
    { _id: '1', title: 'The Rise of AI in Creative Fields', author: 'Dr. Evelyn Reed', date: new Date(Date.now() - 86400000 * 2).toISOString(), readTime: 8, cover: 'https://placehold.co/600x400/1e293b/a5f3fc?text=AI+Creativity', markdown: 'Artificial intelligence is rapidly moving from purely analytical roles into creative domains like design, music composition, and digital art. This shift raises profound questions about originality, intellectual property, and the future role of human artists. Rather than replacement, we are seeing a trend towards augmentation, where AI tools handle the iterative work, allowing human creators to focus on conceptualization and emotional depth. Understanding these tools is paramount for any modern professional. This is a very long text added here to demonstrate the effective expansion and smooth transition of the card content when the "Read More" button is clicked. It should wrap nicely and show all the data when expanded.'},
    { _id: '2', title: 'Navigating the New Job Market Landscape', author: 'Markus Chen', date: new Date(Date.now() - 86400000 * 5).toISOString(), readTime: 5, cover: 'https://placehold.co/600x400/1e293b/38bdf8?text=Job+Market', markdown: 'The post-pandemic job market favors specialized skills and adaptability over general experience. Continuous learning, networking, and demonstrating emotional intelligence (EQ) are now crucial factors for career success. Focus on portfolio building and projects that showcase real-world problem-solving abilities.'},
    { _id: '3', title: 'Mastering Asynchronous Communication', author: 'Sophia Lee', date: new Date(Date.now() - 86400000 * 10).toISOString(), readTime: 4, cover: 'https://placehold.co/600x400/1e293b/06b6d4?text=Communication', markdown: 'In global remote teams, clear, concise, and timely asynchronous communication is a superpower. Avoid unnecessary meetings by prioritizing well-documented decisions and summarizing key outcomes in writing. Good asynchronous practice fosters inclusion and respects different time zones, leading to a more focused and productive work environment.'}
];
...
const MessageToast = ({ message, type }) => {
  ...
  const base = "fixed bottom-5 right-5 p-4 rounded-xl shadow-2xl z-50 flex items-center gap-2 transition-opacity duration-300";
  const map = {
...
const CareerBlogs = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch("https://edumedia-hub-1-bgw0.onrender.com/api/v1/getallblog", {
          headers: {
        ...
        if (res.ok) {
           ...
           if (Array.isArray(apiData)) {
              fetchedData = apiData; // Use real data if successful and is an array
           } else {
        ...
        setError(null); // Clear previous error if mock data is used
      } finally {
  ...
  const bookmark = (_id) => {
    setBlogs((prev) => {
        ...
        return prev.map((b) =>
          b._id === _id ? { ...b, isBookmarked: isNow } : b
        );
    });
  };
    ...
    return (
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {displayBlogs.map((blog) => {
          ...
          const formattedDate = new Date(blog.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
          const coverUrl = blog.cover?.startsWith("http") ? blog.cover : `http://localhost:5001${blog.cover}`;

          return (
            <article
              key={blog._id}
              className={`rounded-3xl bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 overflow-hidden shadow-xl transition-all duration-500 hover:shadow-cyan-500/20 ${open ? 'shadow-2xl border-cyan-500/70' : ''}`}
            >
              {/* Cover Image */}
              <div className="h-48 overflow-hidden relative">
                <img
                  ...
                  onError={(e) => {e.target.onerror = null; e.target.src = `https://placehold.co/600x400/1e293b/38bdf8?text=Career+Insight`;}}
                />
              </div>
